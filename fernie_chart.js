...
var dataset = d3v4.stack()(["male", "female", "mixed"].map(function(fruit) {
  return data.map(function(d) {
    return {x: formatTime(d.year), y: +d[fruit]};
  });
}));

console.log(d3v4.stack().keys(["male", "female", "mixed"])(data))

// Set x, y and colors
var x = d3v4.scaleOrdinal()
  .domain(dataset[0].map(function(d) { return d.x; }))
  .rangeRoundBands([10, width-10], 0.02);

var y = d3v4.scaleLinear()
  .domain([0, d3v4.max(dataset, function(d) {  return d3v4.max(d, function(d) { return d.y0 + d.y; });  })])
  .range([height, 0]);
...
var xAxis = d3v4.svg.axis()
  .scale(x)
  .orient("bottom")
  .tickFormat(d3v4.time.format("%Y"));
...
var rect = groups.selectAll("rect")
  ...
  .on("mousemove", function(d) {
    var xPosition = d3v4.mouse(this)[0] - 15;
    var yPosition = d3v4.mouse(this)[1] - 25;
